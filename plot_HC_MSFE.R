...
glm_TPR_TNR<-read.csv("glm_HC_MSFE_rounds_1_test_TPR_TNR.csv",row.names = 1)
glm_TPR_TNR<-glm_TPR_TNR$x
glm_Sensitivity<-read.csv("glm_HC_MSFE_rounds_1_test_plot_data.csv",row.names = 1)
dim(glm_Sensitivity)
glm_auc<-read.csv("glm_HC_MSFE_rounds_1_test_auc.csv",row.names = 1)
glm_auc<-glm_auc$x

svm_TPR_TNR<-read.csv("svm_HC_MSFE_rounds_1_test_TPR_TNR.csv",row.names = 1)
svm_TPR_TNR<-as.vector(svm_TPR_TNR)
svm_Sensitivity<-read.csv("svm_HC_MSFE_rounds_1_test_plot_data.csv",row.names = 1)
dim(svm_Sensitivity)
svm_auc<-read.csv("svm_HC_MSFE_rounds_1_test_auc.csv",row.names = 1)
svm_auc<-svm_auc$x

logit_TPR_TNR<-read.csv("logit_HC_MSFE_rounds_1_test_TPR_TNR.csv",row.names = 1)
logit_TPR_TNR<-logit_TPR_TNR$x
logit_Sensitivity<-read.csv("logit_HC_MSFE_rounds_1_test_plot_data.csv",row.names = 1)
dim(logit_Sensitivity)
logit_auc<-read.csv("logit_HC_MSFE_rounds_1_test_auc.csv",row.names = 1)
...
jpeg(file=paste0("MSFE_HC_SVM_LR_LREN_test_tmp.jpeg"),width=867/3,height=795/3,units="mm",res=300)
par(mfrow=c(1,1))
plot(xlim=c(0,1),ylim=c(0,1),glm_Sensitivity[,1],glm_Sensitivity[,2],xlab="1-Specificity",ylab="Sensitivity",
     col="red",type='l',cex=0.6,lwd=2,main=("MSFE vs. HC"))
...
jpeg(file=paste0("MSFE_HC_SVM_LR_LREN_test.jpeg"),width=867/3,height=795/3,units="mm",res=300)
par(mfrow=c(1,1))
